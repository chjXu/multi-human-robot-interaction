...
from modules.pose import BODY_PARTS_KPT_IDS, BODY_PARTS_PAF_IDS, Pose_2d
...
def extract_poses(heatmaps, pafs, upsample_ratio, stride, scale, pad):
    ...
    for pose_entry in pose_entries:
        ...
        pose_keypoints = np.ones((num_keypoints, 2), dtype=np.int32) * -1
        for kpt_id in range(num_keypoints):
            if pose_entry[kpt_id] != -1:  # keypoint was found
