...
std::vector<commonFunction::Human_Joint> human_obs;
...
void obsCallback(const geometry_msgs::PointStamped::ConstPtr &obs){
    human_obs.clear();

    Human_Joint human_joint;
    human_joint.x = obs.get()->point.x;
    human_joint.y = obs.get()->point.y;
    human_joint.z = obs.get()->point.z;
    human_obs.push_back(human_joint);
}

double one_order_filtering(double last_vel, double current_vel)
{
    //cout << last_vel << "  " << current_vel << endl;
...
void* goal_rectify__thread(void *arg)
{
    ...
    while(ros::ok()){
        try{
            ...
            if (goal_position.z() < -0.13){
                rectified_position.setZ(-0.13);
...
{   
    ...
    // 监听障碍物位置信息
    ros::Subscriber obs_sub = nh.subscribe("Obstacles", 1, obsCallback);
